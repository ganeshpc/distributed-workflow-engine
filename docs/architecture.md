...
| `engine-api` dependencies | JDK only. No Spring, JPA, Kafka, Jackson, Lombok. JSON travels as `String` (UTF-8 JSON text). | Workers must depend on contracts, not the Boot app and not a JSON library forced through `engine-api`. The Kafka task and result codec is `engine-json`, which uses Jackson. |
